...
// We store 256 bit for x and d values to support up to 256-bit range widths

typedef struct {

  int256_t  x;    // Position of kangaroo (256-bit)
  int256_t  d;    // Travelled distance (256-bit)
  uint32_t  kType; // Kangaroo type (TAME/WILD)
...
public:
  ...
  int Add(uint64_t h,int256_t *x,int256_t *d,uint32_t type);
...
private:
  ...
  ENTRY *CreateEntry(int256_t *x,int256_t *d,uint32_t type);
